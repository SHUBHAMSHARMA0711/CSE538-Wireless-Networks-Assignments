 ...
 */
void setupp(std::string dlAckSeqType);
void serverMaker();
void clientMaker();
...
int pages = 0;
float pageLoadTimes = 0.0;
std::vector<Time> mainObjectGenerationTime;
std::vector<Time> timeAtWhichClientReceivedMainObject;
...
void MainObjectGenerated(uint32_t size)
{
    ...
    mainObjectGenerationTime.push_back(Simulator::Now());
}
...
void ClientMainObjectReceived(Ptr<const ThreeGppHttpClient>, Ptr<const Packet> packet)
{
    ...
    {
        NS_LOG_INFO("Client has successfully received a main object of " << p->GetSize() << " bytes.");
        timeAtWhichClientReceivedMainObject.push_back(Simulator::Now());
    }
...
void ClientPageReceived(Ptr<const ThreeGppHttpClient> client, const Time& time, uint32_t numObjects, uint32_t numBytes)
{
    ...
    out << time.As(Time::MS);
    pageLoadTimes += std::stof(out.str().substr(1,5));
    ++pages;
}
  ...
  for (uint16_t bss = 0; bss < m_nObss + 1; bss++)
    {
      ...
      if (m_enablePcap)
        {
          phy.EnablePcap ("/home/shubham21099/WN/Assignment_3/Part-A/PCAPs/STA_pcap", staDevices);
          phy.EnablePcap ("/home/shubham21099/WN/Assignment_3/Part-A/PCAPs/AP_pcap", apDevice);
        }
    }
...
{
    /* Setting HTTP Server*/
    ThreeGppHttpServerHelper serverHelper(apNodeInterface.GetAddress(0));
    ApplicationContainer serverApps = serverHelper.Install(wifiApNode.Get(0));
    ...
    /* Setting HTTP Server Attributes*/
    PointerValue varPtr;
...
void setupp(std::string dlAckSeqType)
{
    ...
    WifiHelper wifi;
    wifi.SetStandard(WIFI_STANDARD_80211ax);
    wifi.ConfigHeOptions("GuardInterval", TimeValue(NanoSeconds(3200)), "MpduBufferSize",UintegerValue(64));
    wifi.SetRemoteStationManager("ns3::ConstantRateWifiManager", "DataMode", StringValue("HeMcs" + mcs), "ControlMode", StringValue("OfdmRate24Mbps"));
    ...
    phy.SetChannel(spectrumChannel);
    phy.SetPcapDataLinkType(WifiPhyHelper::DLT_IEEE802_11_RADIO);
    phy.Set("ChannelSettings", StringValue("{0, " + std::to_string(40) + ", " + "BAND_5GHZ, 0}"));


    WifiMacHelper mac;
    mac.SetType("ns3::StaWifiMac", "Ssid", SsidValue(Ssid("ns3-80211ax")));
    NetDeviceContainer staDevices = wifi.Install(phy, mac, wifiStaNodes);

    if (dlAckSeqType == "NO-OFDMA")
    {
        mac.SetMultiUserScheduler("ns3::RrMultiUserScheduler",
                                  "NStations", UintegerValue(18),
                                  "EnableUlOfdma", BooleanValue(false),
                                  "EnableBsrp", BooleanValue(false),
                                  "AccessReqInterval", TimeValue(accessReqInterval));
    }
    
    mac.SetType("ns3::ApWifiMac", "EnableBeaconJitter", BooleanValue(false), "Ssid", SsidValue(Ssid("ns3-80211ax")));
    ...
    positionAlloc->Add(Vector(radius, 0.0, 0.0));
    mobility.SetPositionAllocator(positionAlloc);
    ...
    Ipv4AddressHelper address;
    address.SetBase("169.1.0.0", "255.255.255.0");
    ...
    phy.EnablePcap("/home/shubham21099/WN/Assignment_3/Part-B/PCAPs/AP_pcap", apDevice);
    phy.EnablePcap("/home/shubham21099/WN/Assignment_3/Part-B/PCAPs/STA_pcap", staDevices);
    ...
    Time pagesLatency = Seconds(0);

    for (size_t i = 0; i < mainObjectGenerationTime.size(); ++i) 
    {
        pagesLatency += timeAtWhichClientReceivedMainObject[i] - mainObjectGenerationTime[i];
    }

    std::cout << "\nAverage Page load time: " << pageLoadTimes / pages << " ms" << std::endl << std::endl;
    std::cout << "Average Main Object Latency: " << pagesLatency / mainObjectGenerationTime.size() << std::endl << std::endl;
}
...
void clientMaker()
{
    ...
    {
        ThreeGppHttpClientHelper clientHelper(apNodeInterface.GetAddress(0));
